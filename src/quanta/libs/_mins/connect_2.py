...
class main(type('recommand_settings', (), config.minfreq_settings.key), db):
    ...
    def __read_from_internal__(self, dates):
        adds = dates.dt.date
        if hasattr(self, '_internal_data'):
            exists = np.unique(self._internal_data.index.date)
            adds = np.setdiff1d(adds, exists)
            if len(adds):
                df = pd.concat([self._internal_data] + [self.__read_from_db__(i) for i in adds])
            else:
                df = self._internal_data
            df = df[np.isin(df.index.date, dates.dt.date)]
        else:
            df = pd.concat([self.__read_from_db__(i) for i in adds])
        df = df.sort_index()
    ...
    def windows(self):
        if not hasattr(self, '_window'):
            if not hasattr(self, '_internal_data'):
                ...
                self._window = np.unique(self._internal_data.index.date).shape[0]
        return self._window
